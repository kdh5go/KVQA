...
class BCNet(nn.Module):
    ...
    def __init__(self, v_dim, q_dim, h_dim, h_out, act='ReLU', dropout=[.2, .5], k=3):
        ...
        if None == h_out:
            ...
            self.h_mat = nn.Parameter(torch.Tensor(
                1, h_out, 1, h_dim * self.k).normal_())
    ...
    def forward(self, v, q):
        ...
        # epoch 1, time: 157.84
            ...
            logits = torch.matmul(h_, q_.unsqueeze(
                1).transpose(2, 3))  # b x h_out x v x q
        ...
        # epoch 1, time: 304.87
            ...
            q_ = self.q_net(q).transpose(1, 2).unsqueeze(2)
            d_ = torch.matmul(v_, q_)  # b x h_dim x v x q
            logits = self.h_net(d_.transpose(
                1, 2).transpose(2, 3))  # b x v x q x h_out
    ...
    def forward_with_weights(self, v, q, w):
        ...
        logits = torch.matmul(torch.matmul(
            v_, w.unsqueeze(1)), q_)  # b x d x 1 x 1
