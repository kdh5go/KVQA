...
def initialize_exp(params):
    ...
    json.dump(vars(params), open(os.path.join(
        exp_folder, 'params.pkl'), 'w'), indent=4)
    ...
    logger = create_logger(os.path.join(
        exp_folder, 'train.log'), rank=getattr(params, 'global_rank', 0))
...
def get_dump_path(params):
    ...
    assert not params.log_dir in ('', None), \
        'Please choose your favorite destination for dump.'
    log_dir = params.log_dir

    # create the sweep path if it does not exist
    when = date.today().strftime('%m%d-')
    sweep_path = os.path.join(params.log_dir, when + params.exp_name)
