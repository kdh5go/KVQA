...
class Metrics:
    ...
    def batch_accuracy_10(self, predicted, true):
        ...
        agreeing_all = torch.zeros(
            [predicted.shape[0], 1], dtype=torch.float).cuda()
        for i in range(10):
            tmp = ok[:, i].reshape(-1, 1)
            agreeing_all += true.gather(dim=1, index=tmp)
            if i == 0:
                top1 = (agreeing_all).clamp(max=1)
            if i == 2:
                top3 = (agreeing_all).clamp(max=1)
            if i == 9:
                top10 = (agreeing_all).clamp(max=1)
    ...
    def batch_accuracy_50(self, predicted, true):
        ...
        for i in range(50):
            ...
            if i == 9:
                top10 = (agreeing_all).clamp(max=1)
            if i == 29:
                top30 = (agreeing_all).clamp(max=1)
            if i == 49:
                top50 = (agreeing_all).clamp(max=1)
    ...
    def batch_mr_mrr(self, predicted, true):
        ...
        _, predict_ans_rank = predicted.topk(
            top_rank, dim=1)  # 答案排名的坐标 batchsize * 500
        ...
        # 此时为0的位置就是预测正确的位置
        _, real_ans_list = ans_different.topk(
            top_rank, dim=1)  # 此时最后一位的数值就是正确答案在预测答案里面的位置,为 0
